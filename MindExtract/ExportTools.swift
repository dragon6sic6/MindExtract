...
final class PDFExporter: NSObject, WKNavigationDelegate {
    ...
    /// Base64 data-URI for a logo image, for embedding in the PDF letterhead.
    static func logoDataURI(path: String) -> String? {
    ...
    static func transcriptHTML(title: String,
        ...
        * { -webkit-print-color-adjust: exact; }
        ...
        .letterhead { display:flex; align-items:center; gap:12px; border-bottom:2px solid #e5e5ea; padding-bottom:12px; margin:0 0 18px; }
        .letterhead img { max-height:44px; max-width:160px; }
        .letterhead .brand { font-size:16px; font-weight:700; color:#1d1d1f; }
        </style></head><body>
        \({
            let name = brandName?.trimmingCharacters(in: .whitespaces) ?? ""
            if name.isEmpty && logoDataURI == nil { return "" }
            let img = logoDataURI.map { "<img src=\"\($0)\">" } ?? ""
            let brand = name.isEmpty ? "" : "<span class='brand'>\(esc(name))</span>"
            return "<div class='letterhead'>\(img)\(brand)</div>"
        }())
        <h1>\(esc(title))</h1>
